  ...
  setupTabs: (container, className = 'highlightItem') => {
    ...
    switchList.addEventListener('click', (event) => {
      ...
      if (highlightItems.length > index) {
        ...
        if (
          highlightContentItems[index].closest('.zoomin-effect')
          && index !== activeTab
        ) {
          const moreContent = highlighContentItem.querySelector('.more-content');
          const moreContentExpanded = highlighContentItem.querySelector(
          ...
          if (!isContentVisible) {
            ...
            moreContentExpanded.style.display = isContentVisible
              ? 'block'
              : 'none';
    ...
    // Initial setup
    const defaultHighlightItem = container.querySelector(
      `.${className}.switch-index-0`,
    );
    ...
    if (
      defaultHighlightItem
        ?.closest('.highlight-tabs')
        ?.classList?.contains('no-zoomin-effect')
    ) {
      setTimeout(() => {
        prevContenHeight = `${
          defaultHighlightItem.querySelector('.highlightItem-content')
            .scrollHeight
        }px`;
      }, 500);
    }
