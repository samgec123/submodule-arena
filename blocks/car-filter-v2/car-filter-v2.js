...
export default async function decorate(block) {
  const [titleEl, priceTextEl, selectVariantEl, filterSelectEl] = block.children;
  let cars = [];
  const {
    publishDomain,
    allFilterText,
  ...
  function carModelInfo(result) {
    ...
    carCardsWithTeaser.classList.add('card-list-teaser', 'splide-carousel', 'splide');

    const newContainer = document.createElement('div');
    newContainer.classList.add('filter-cars', 'container');

    const carFiltersTabWarpper = document.createElement('div');
    carFiltersTabWarpper.classList.add('car-filter-tab-wrapper');

    const carFiltersContainer = document.createElement('div');
    carFiltersContainer.classList.add('car-filter-list');
    carFiltersTabWarpper.appendChild(carFiltersContainer);
    ...
    carFiltersTabWarpper.append(carouselArrows);
    ...
    carCardsTrack.appendChild(carCardsContainer);

    carCardsWithTeaser.appendChild(carFiltersTabWarpper);
    carCardsWithTeaser.appendChild(carCardsTrack);
      ...
      carsToRender.forEach((car, index) => {
        ...
        card.addEventListener('click', () => {
          window.adobeDataLayer.push(utility.createLinkClicksDataLayerObject({
            ...
            cityName: utility.getLocalStorage('selected-location')?.cityName || 'Delhi',
            authenticatedState: 'unauthenticated',
            componentName: heading?.innerText || '',
            componentType: 'Tile',
            componentTitle: title,
            webInteractionName: heading?.innerText || '',
