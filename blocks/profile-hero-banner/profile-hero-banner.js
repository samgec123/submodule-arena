...
export default async function decorate(block) {
  ...
  const bannerLink1 = bannerLink1Url ? `<li><a href="${bannerLink1Url}">${bannerLink1Text}</a></li>` : '';
  const bannerLink2 = bannerLink2Url ? `<li><a href="${bannerLink2Url}">${bannerLink2Text}</a></li>` : '';
  const bannerLink3 = bannerLink3Url ? `<li><a href="${bannerLink3Url}">${bannerLink3Text}</a></li>` : '';
  const bannerLink4 = bannerLink4Url ? `<li><a href="${bannerLink4Url}">${bannerLink4Text}</a></li>` : '';
  const bannerLink5 = bannerLink5Url ? `<li><a href="${bannerLink5Url}">${bannerLink5Text}</a></li>` : '';

  const pageTitle = document.title || '';
  const navigationBackUrl = window.location.pathname.split('/').slice(0, -1).join('/');

  const response = await apiUtils.fetchCustomerData();

  const username = response?.data?.[0]?.customer?.customerName || '';
  const city = response?.data?.[0]?.customer?.residentialAddressSale?.cityDesc || '';
  const state = response?.data?.[0]?.customer?.residentialAddressSale?.stateDesc || '';
  ...
  const initDataLayerEvents = async () => {
    ...
    const hashedPhone = await utility.getSHA256Hash(mobile);

    navLinks.forEach((link) => {
      link.addEventListener('click', (e) => {
        window.adobeDataLayer.push(utility.createLinkClicksDataLayerObject({
          selectedLanguage: document.documentElement.lang,
          cityName: city,
          authenticatedState: 'authenticated',
          hashedphoneSHA256: hashedPhone,
          componentType: e.target.innerText,
          componentName: 'CTA',
          componentTitle: 'Profile Hero Banner',
          webInteractionName: e.target.innerText,
          webInteractionType: 'CTA',
