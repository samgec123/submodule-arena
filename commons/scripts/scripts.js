...
function buildMultiStepForms(main) {
  const multiStepForms = ['sf-journey-start', 'loan-application'];
...
function loadMartech() {
  import(`${window.hlx.codeBasePath}/scripts/gtm.js`).catch(() => { });
}
...
function loadDelayed() {
  ...
  window.setTimeout(() => {
    ...
    martechDelayed();
    import('./delayed.js');
    if (getMartechDelayParam() === 'delayed') {
      loadMartech();
    }
  }, 3000);
