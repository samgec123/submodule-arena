...
const authUtils = {
  isAuthPage: () => getMetadata('isauthenticationrequired') === 'true',
  policies: async () => {
    ...
    return new Promise((resolve, reject) => {
      ...
      script.onload = async () => {
        try {
          ...
          const instance = await (msal.PublicClientApplication.
            createPublicClientApplication(await authUtils.config()));
  ...
  handleRedirect: async () => {
    ...
    try {
      ...
      if (accounts.length >= 1) {
        ...
        const currentAccounts = accounts.filter((account) => account.homeAccountId.toUpperCase()
          .includes(policies.signUpSignInPolicy.toUpperCase())
        ...
        if (currentAccounts.length > 1) {
          if (currentAccounts.every(
            (account) => account.localAccountId === currentAccounts[0].localAccountId,
          )) {
  ...
  waitForAuth: async () => new Promise((resolve) => {
