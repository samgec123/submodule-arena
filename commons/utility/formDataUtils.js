...
const formDataUtils = {
  ...
  fetchSfFormData: async (formType, prefix = 'default') => {
    ...
    try {
      const formData = await getFormData(`${prefix === 'default' ? '' : prefix}/common-fields.json`);
      const specificFormData = await getFormData(`${prefix === 'default' ? '' : prefix}/${formType}.json`);
      Object.keys(specificFormData).forEach((item) => {
        formData[item] = specificFormData[item];
      });
  ...
  fetchBankFieldMap: async (formType, prefix = 'default') => {
    ...
    try {
      const formData = await getFormData(`${prefix === 'default' ? '' : prefix}/${formType}.json`);
      return formData;
    } catch (error) {
      return {};
    }
  ...
  createDropdown: (
    ...
    return `
      <div class="form-dropdown ${className} form-field ${style ? `dropdown-${style}` : ''}">
        ${labelObj.label}
        <select class="form-dropdown-select" id="${id}" name="${name}" aria-labelledby="${id}-label" ${requiredMessage ? 'required=true' : ''} ${attributeString}>
  ...
  createInputField: (
    ...
    return `
      <div class="form-input ${className} form-field ${style ? `input-${style}` : ''}">
        ${labelObj.label}
        <input class="${inputClassName}" type="${type}" id="${id}" name="${name}" placeholder="${labelObj.placeholder}" ${attributesString} aria-labelledby="${id}-label" ${requiredMessage ? 'required' : ''} autocomplete="off" />
        ${requiredMessage ? `<span class="validation-text validation-required">${requiredMessage}</span>` : ''}
  ...
  createDobField: (data, className = 'half-width', attributes = {}, style = '', inputClassName = '') => {
    ...
    return `
      <div class="form-input ${className} form-field">
          <input class="${inputClassName} form-control" placeholder="${labelObj.placeholder}" type="date" ${attributesString} name="${name}" tabindex="17">
          ${requiredMessage ? `<span class="validation-text validation-required">${requiredMessage}</span>` : ''}
          ${validationMessage ? `<span class="validation-text validation-pattern" id="${id}-validation">${validationMessage}</span>` : ''}
      </div>
    `;
